...
def preprocess(df, feature, label_column, test_size=0.15, validation_size=0.15, batch_size=128):
    ...
    df_train, df_temp=train_test_split(df, test_size=(test_size + validation_size), stratify=df[label_column],
                                       random_state=42)
    df_val, df_test=train_test_split(df_temp, test_size=test_size / (test_size + validation_size),
                                     stratify=df_temp[label_column], random_state=42)
    ...
    y_train, y_val, y_test=map(lambda x: tensorize(x, torch.long),
                               [df_train[label_column], df_val[label_column], df_test[label_column]])
    tte_train, tte_val, tte_test=map(lambda x: tensorize(x, torch.float32),
                                     [df_train[tte_column], df_val[tte_column], df_test[tte_column]])

    # Step 8: Compute Class Weights with Continuous Scaling
    class_weight_tensor=None
    if df_train[label_column].nunique() > 1:
        class_weight=compute_class_weight("balanced", classes=np.unique(df_train[label_column]),
        ...
        print(
            f"Event Rate: {event_rate:.4f}, Scaling Factor: {scale_factor:.2f}, Adjusted Class Weights: {class_weight_tensor.cpu().numpy()}")
...
class FocalCoxLoss(nn.Module):
    def __init__(self, class_weight, gamma=2, eps=1e-8, tte_scaling="tanh"):
        """
        Combined Focal Loss and Cox Loss with uncertainty weighting.

        Args:
            class_weight (torch.Tensor): Class weights for Focal Loss.
            ...
            tte_scaling (str): Method to scale TTE values ("none", "tanh"). Default is "none".
        """
        super(FocalCoxLoss, self).__init__()

        # Register class weights as buffer (ensures it's on the right device)
        ...
        # Learnable log(sigma) parameters for uncertainty weighting
        self.log_sigma_focal=nn.Parameter(torch.tensor(-2.0), requires_grad=True)
        self.log_sigma_cox=nn.Parameter(torch.tensor(3.0), requires_grad=True)

    def forward(self, outputs, labels, tte_scores, tte):
        """
        Compute the combined Focal Loss and Cox Loss using uncertainty weighting.
        """
        ...
        # -------------------- Process TTE Scores --------------------
        if self.tte_scaling == "tanh":
            tte_scores=torch.tanh(tte_scores)  # 限制范围，防止数值不稳定

        # -------------------- Compute Cox Loss (DeepSurv Style) --------------------
        risk_scores=tte_scores.exp()  # 计算风险分数 e^η
        cum_risk_scores=torch.cumsum(risk_scores, dim=0)  # 累加计算风险
        log_hazard_ratio=tte_scores - torch.log(cum_risk_scores + self.eps)  # 避免 log(0)
        cox_loss=-torch.sum(log_hazard_ratio * labels) / (labels.sum() + self.eps)

        # -------------------- Apply Learnable Sigma (保持不动) --------------------
        ...
        total_loss+=self.log_sigma_focal.detach() + self.log_sigma_cox.detach()  # 继续优化 sigma
...
class BERTMLPClassifier(nn.Module):
    ...
    This model consists of:
        ...
        - **Final Classifier:** Gradually reduces feature dimensionality before classification.
        - **TTE Predictor:** Estimates risk scores for survival analysis tasks.
    ...
    def __init__(self, bert_model, num_continuous_features, dropout_rate=0.5, num_attention_heads=6,
        ...
        self.classifier=nn.Sequential(
            nn.Linear(self.bert_hidden_size, self.classifier_hidden_sizes[0]),  # 768 → 384
            nn.ReLU(),
            nn.Dropout(dropout_rate),
            nn.Linear(self.classifier_hidden_sizes[0], self.classifier_hidden_sizes[1]),  # 384 → 192
            nn.ReLU(),
            nn.Dropout(dropout_rate),
            nn.Linear(self.classifier_hidden_sizes[1], 2)  # 192 → 2 (Binary classification)
        )

        # 7. TTE Predictor (Survival Risk Score)
        self.tte_predictor=nn.Linear(self.bert_hidden_size, 1)  # Predict TTE risk score
    ...
    def forward(self, text_input_ids, text_attention_mask, continuous_features):
        ...
        logits=self.classifier(attended_features)  # Shape: (batch_size, 2)

        # Step 7: TTE Risk Score Prediction
        tte_scores=self.tte_predictor(attended_features)  # Shape: (batch_size, 1)

        # Step 8: Apply Fixed Temperature Scaling to Logits
        scaled_logits=logits / self.temperature

        return scaled_logits, tte_scores
...
def calculate_best_f1(labels, probs, threshold_range=(0.1, 0.9, 0.01), mode='f1'):
    ...
    best_weighted_f1, best_threshold_weighted_f1=weighted_f1_scores[best_idx_weighted_f1], thresholds[
        best_idx_weighted_f1]
...
def BERTMLP_run(gender, path, features, label='cvd_q', batch=128, epoch=10, freeze=20, patience=3,
    ...
    temp_model_path=f"{path}/saved/{gender}/{label}/temp_model.pt"
    ...
    try:
        ...
        if search_trials > 0:
            def objective(trial):
                ...
                if val_metrics['best_f1'] > best_val_f1 or (
                        val_metrics['best_f1'] == best_val_f1 and val_metrics['auc'] > best_val_auc - tolerance):
        ...
        if search_trials > 0:
            ...
            best_model.load_state_dict(torch.load(temp_model_path, map_location=device))
